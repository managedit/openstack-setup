...
# Glance Setup
mysql -h $MYSQL_HOST -u$MYSQL_USER -p$MYSQL_ROOT_PASS -e 'DROP DATABASE IF EXISTS glance;'
mysql -h $MYSQL_HOST -u$MYSQL_USER -p$MYSQL_ROOT_PASS -e 'CREATE DATABASE glance;'

echo "GRANT ALL ON glance.* TO 'glance'@'%' IDENTIFIED BY '$MYSQL_GLANCE_PASS';" | mysql -h $MYSQL_HOST -u$MYSQL_USER -p$MYSQL_ROOT_PASS
